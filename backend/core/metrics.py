...
def compute_strategy_report(session: Session, *, symbol: str, exchange: str, category: str, strategy_id: Optional[str]) -> Dict[str, Any]:
    ...
    if not ser:
        return {"equity": [], "sharpe": 0.0, "calmar": 0.0, "maxdd": 0.0, "winrate": 0.0, "pnl_total": 0.0}
    ...
    ann_return = mu * 252
    sh = sharpe(rets, 252)
    cal = calmar(ann_return, mdd)
    rr = _realized_rows(session, symbol=symbol, exchange=exchange, category=category, strategy_id=strategy_id)
    wins = sum(1 for r in rr if r.pnl>0)
    losses = sum(1 for r in rr if r.pnl<=0)
    winrate = (wins / max(1, wins+losses)) if rr else 0.0
    pnl_total = sum(r.pnl for r in rr)
    return {"equity": [{"ts": s.ts, "equity": s.equity} for s in ser], "sharpe": sh, "calmar": cal, "maxdd": mdd, "winrate": winrate, "pnl_total": pnl_total}
